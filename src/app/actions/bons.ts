...
export async function createBon(data: BonFormValues) {
  try {
    ...
    const { nomorBon, tanggal, customerId, ongkir, deskripsi, isBonus, lineItems } = validated.data;
    ...
    const newBon = await prisma.$transaction(async (tx) => {
      ...
      // Jika isBonus, potong Ledger
      if (isBonus) {
        await tx.bonusLedger.create({
          data: {
            customerId,
            bonId: createdBon.id,
            bonusCount: 1, // Anggap 1 bon menghabiskan 1 bonus
            omzetConsumed: customer.bonusThreshold,
          },
        });
      }
